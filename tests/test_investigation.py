...
class RunJsonTest(unittest.TestCase):
    ...
    def test_returns_parsed_rows_on_success(self):
        ...
        inv.configure(
            ...
            q_errors="Q1",
            q_soc_log_spike=_fixed("Q2"),
            q_trace_find_errors=_fixed("Q3"),
        )
...
class StartNodeTest(unittest.TestCase):
    ...
    def test_normal_rate_concludes(self):
        ...
        text, is_error, next_node, next_service = inv.run_error_rate_node("start", "1h ago", None)
        self.assertFalse(is_error)
        self.assertIsNone(next_node)
...
class CheckLogSpikeNodeTest(unittest.TestCase):
    def setUp(self):
        ...
        inv.configure(
            ...
            since_hours=lambda s: 1.0,
            q_errors="Q_ERRORS",
            q_soc_log_spike=self._q_soc_log_spike,
            q_trace_find_errors=_fixed("Q_TRACE"),
        )
        self.responses = {}
    ...
    def test_no_service_param_is_a_validation_error(self):
        text, is_error, next_node, next_service = inv.run_error_rate_node("check_log_spike", "1h ago", None)
        self.assertTrue(is_error)
        self.assertIsNone(next_node)
    ...
    def test_spike_advances_to_check_traces(self):
        ...
        hits = [2] * 55 + [20] * 5
        self._spike_response("Q_SPIKE[checkout]", "checkout", hits)
        text, is_error, next_node, next_service = inv.run_error_rate_node("check_log_spike", "1h ago", "checkout")
        self.assertFalse(is_error)
...
class CheckTracesNodeTest(unittest.TestCase):
    ...
    def test_no_service_param_is_a_validation_error(self):
        text, is_error, next_node, next_service = inv.run_error_rate_node("check_traces", "1h ago", None)
        self.assertTrue(is_error)
        self.assertIsNone(next_node)
    ...
    def test_query_is_scoped_to_the_requested_service(self):
        ...
        self.responses["Q_TRACE[checkout]"] = (
            bzrk_json_table(["trace_id", "span_name", "timestamp", "service"], rows),
            False,
        )
    ...
    def test_failing_traces_found_concludes_with_root_cause_verdict(self):
        ...
        text, is_error, next_node, next_service = inv.run_error_rate_node("check_traces", "1h ago", "checkout")
        self.assertFalse(is_error)
        self.assertIsNone(next_node)
    ...
    def test_no_matching_traces_concludes_with_ingestion_gap_hypothesis(self):
        self.responses["Q_TRACE[checkout]"] = (
            bzrk_json_table(["trace_id", "span_name", "timestamp", "service"], []),
            False,
        )
        text, is_error, next_node, next_service = inv.run_error_rate_node("check_traces", "1h ago", "checkout")
        self.assertFalse(is_error)
...
class FullWalkTest(unittest.TestCase):
    def setUp(self):
        inv.configure(
            ...
            since_hours=lambda s: 1.0,
            q_errors="Q_ERRORS",
            q_soc_log_spike=lambda service: f"Q_SPIKE[{service}]",
    ...
    def test_full_walk_elevated_spike_traces_found(self):
        ...
        self.responses["Q_SPIKE[checkout]"] = (
            bzrk_json_table(["service", "hits"], [["checkout", [2] * 55 + [20] * 5]]),
            False,
        )
        self.responses["Q_TRACE[checkout]"] = (
            bzrk_json_table(
        ...
        text2, err2, next2, svc2 = inv.run_error_rate_node("check_log_spike", "1h ago", svc1)
        self.assertFalse(err2)
        self.assertEqual(next2, "check_traces")
        self.assertEqual(svc2, "checkout")

        text3, err3, next3, svc3 = inv.run_error_rate_node("check_traces", "1h ago", svc2)
    ...
    def test_full_walk_elevated_but_no_spike_early_exit(self):
        self.responses["Q_ERRORS"] = (bzrk_json_table(["service", "errors"], [["checkout", 700]]), False)
        self.responses["Q_SPIKE[checkout]"] = (bzrk_json_table(["service", "hits"], [["checkout", [5] * 60]]), False)
    ...
    def test_branch_logic_unaffected_by_reformatted_but_same_structured_value(self):
        ...
        for variant in (variant_a, variant_b):
            with self.subTest(variant=variant[:30]):
                ...
                _, is_error, next_node, next_service = inv.run_error_rate_node("start", "1h ago", None)
                self.assertFalse(is_error)
                self.assertEqual(next_node, "check_log_spike")
                self.assertEqual(next_service, "checkout")
