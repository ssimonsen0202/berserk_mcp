...
| `BERSERK_LOCAL_ONLY` | `0` | Refuses OpenAI and Anthropic outright for the generation ladder and any direct `llm_complete()` call, even if their API keys are present. Also activates a destination allowlist (`BERSERK_EGRESS_ALLOWED_HOSTS`/`_CIDRS`) for every outbound integration: Hermes, CanonLoom, Discord, the eval harness. Loopback is always allowed. Run `berserk-mcp --doctor` to see the effective policy. |
| `BERSERK_EGRESS_ALLOWED_HOSTS` | unset | Approved non-loopback destination hostnames (comma-separated). Enforced when `BERSERK_LOCAL_ONLY` is on, or when this (or `BERSERK_EGRESS_ALLOWED_CIDRS`) is itself set — setting either opts into the restrictive policy even with local-only off. |
| `BERSERK_EGRESS_ALLOWED_CIDRS` | unset | Approved non-loopback destination networks (comma-separated CIDRs); same activation rule as `BERSERK_EGRESS_ALLOWED_HOSTS`, for IP ranges. |
| `BERSERK_MCP_MGMT_TOKEN` | unset | Shared token for `save_query` and `approve_generated_query`. Unset by default for backwards compatibility; when set, callers must pass its value as the `mgmt_token` argument. Store it as a secret and use a long random value. |
