...
`validate_kql` and the arbitrary `search` path use the same static validator.
Static validation is a deterministic cost estimate, not an engine proof: it
cannot know the load generated by other tenants, current shard placement, or
whether a different concurrent query is saturating the cluster.
...
| `SOURCE_INTRODUCING_OPERATOR` | blocked in every validation mode, because the execution boundary (`_kql_boundary.source_violation`) enforces the same rule before `bzrk` runs: source operators and functions (`union`, `join`, `lookup`, `evaluate`, `find`, `search`, `invoke`, `externaldata`, `let`, `cluster(`, `database(`, `table(`, `toscalar(`, `materialize(`, `view(`, and similar) anywhere outside string literals; and any right operand of `in`/`!in`/`in~`/`has`/`has_any`/`has_all` and the other string operators that is not a literal value. A literal operand is strings, numbers, `true`/`false`/`null`, and `dynamic(...)`/`datetime(...)`-style literals; a bare or bracket-quoted name or a function call there can be a table, so `where x in (Secret)`, `where x has Secret` and `where x has SecretFn()` are refused, and so is a column-to-column comparison such as `where x has y`. |
...
small `take`/`top`/`tail` bounds, and narrow `project` columns lower the score
but do not erase expensive operators.

Live validation is opt-in:

```json
{
  "kql": "default | where resource['service.name'] == 'api' | count",
  "since": "15m ago",
  "mode": "live"
}
```

It first runs static validation, refuses blocking errors, then executes one
bounded read-only query if `BERSERK_MCP_KQL_LIVE_VALIDATION=1`. The response
adds a `runtime` receipt:
...
`duration_ms` is measured locally with `time.monotonic()`. Rows, bytes, and
engine stats are populated only when the `bzrk` CLI returns a supported stats
format; otherwise they remain `null` and the report includes
`STATS_UNAVAILABLE`.

Useful prompts:

- “Validate this KQL before I run it.”
- “Why is this query expensive?”
- “Make this query safer and faster.”
- “Run a live validation and show me the engine cost.”
