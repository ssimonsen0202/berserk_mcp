...
- **Generated KQL rejects semicolons unconditionally.** Whether Berserk's backend executes semicolon-separated multi-statement KQL can't be verified without a live authenticated check, so `validate_generated_query` rejects any semicolon anywhere in a generated query — including inside a quoted string literal — rather than assume any placement is safe.
- **A malformed LLM provider reply can't crash the generation pipeline.** A provider can legitimately return `content: null` (common for a tool-call-only reply) without raising any exception in `llm_complete` — the key exists, its value just isn't text. `_parse_generated_reply` now validates the type before use instead of calling `.strip()` on it, which previously raised an unhandled `AttributeError` that could terminate the whole `--worker` loop instead of failing just the one job.
- **`BERSERK_MAX_AUTOQUEUE` is clamped, not just parsed.** A negative value used to reach a Python list slice (`list[:N]`) unchanged — `list[:-1]` means "all but the last one," which would queue *nearly everything* instead of nothing, inverting the flood-control invariant this constant exists to enforce. Now: negative clamps to 0, an absurdly large value is capped at a hard ceiling (500), and an unparseable value falls back to the documented default (5).
- **CI and repository hygiene.** GitHub Actions steps are pinned to a full commit SHA (not a mutable tag), the workflow declares `permissions: contents: read` explicitly, and the checkout step sets `persist-credentials: false` so the runner never retains a token past the job. `setuptools` is version-bounded (`>=61,<90`) instead of open-ended. `.gitignore` now excludes `.env*` files, private-key file extensions, and every local generated store this project creates (`discovery_queue.json`, `known_sources.json`, `schema_knowledge.json`, `amendments_log.json`, `llm_config.json`, lock files, temp files) — not just `learned.json`.
